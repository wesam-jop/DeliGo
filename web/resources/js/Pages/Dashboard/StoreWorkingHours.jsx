...
export default function StoreWorkingHours({ store, workingHours: initialWorkingHours }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        form.put('/dashboard/store/working-hours', {
            preserveScroll: true,
        });
    };
    ...
    return (
        <StoreLayout title={t('working_hours') || 'أوقات الدوام'} subtitle={t('manage_working_hours') || 'إدارة أوقات الدوام للمتجر'}>
                ...
                <div className="rounded-3xl border border-slate-200 bg-white shadow-sm p-6">
                    <div className="mb-6">
                        ...
                        <p className="text-sm text-slate-500 mt-2">
                            {t('set_store_working_hours_description') || 'حدد أوقات العمل لكل يوم من أيام الأسبوع. يمكنك تعطيل أي يوم بإغلاقه.'}
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            return (
                                ...
                                >
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                        ...
                                        {!isClosed && (
                                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-1 sm:ml-4">
                                                <div className="flex items-center gap-2 flex-1">
                                                    <label className="text-sm font-medium text-slate-700 whitespace-nowrap">
                                                        {t('opening_time') || 'وقت الفتح'}:
                                                    </label>
                                                    <input
                                                        type="time"
                                                        value={hourData?.opening_time || '09:00'}
                                                        onChange={(e) => handleTimeChange(day.key, 'opening_time', e.target.value)}
                                                        className="rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-900 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                                                    />
                                                </div>
                                                <span className="text-slate-400">-</span>
                                                <div className="flex items-center gap-2 flex-1">
                                                    <label className="text-sm font-medium text-slate-700 whitespace-nowrap">
                                                        {t('closing_time') || 'وقت الإغلاق'}:
                                                    </label>
                                                    <input
                                                        type="time"
                        ...
                        <div className="pt-4 border-t border-slate-200">
                            <button
                                ...
                                className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-purple-600 via-indigo-500 to-blue-500 px-6 py-3 text-sm font-semibold text-white shadow-lg hover:shadow-xl transition disabled:opacity-50"
                            >
