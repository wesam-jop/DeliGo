...
export default function Customers({ customers, stats }) {
    ...
    const filteredCustomers = customers?.filter(customer => {
        ...
        const matchesFilter = 
            ...
            (filterStatus === 'with_orders' && customer.orders_count > 0);
        
        return matchesSearch && matchesFilter;
    }) || [];

    const handleDelete = (customer) => {
        const message = t('confirm_delete_customer') || 'Are you sure you want to delete this customer?';
        if (confirm(message)) {
        ...
        <AdminLayout title={t('customers')}>
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                        <h2 className="text-3xl font-bold text-slate-900">{t('customers')}</h2>
                        <p className="text-slate-600 mt-2">{t('manage_customers') || 'Manage all customer accounts'}</p>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            <span className="font-semibold">{t('add_customer') || 'Add Customer'}</span>
                        </Link>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl shadow-sm border border-blue-200 p-6 hover:shadow-md transition-all group">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-blue-600 mt-1">{t('all_customers') || 'All registered'}</p>
                            </div>
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl shadow-sm border border-green-200 p-6 hover:shadow-md transition-all group">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-green-700">{t('verified_customers') || 'Verified'}</p>
                                <p className="text-3xl font-bold text-green-900 mt-2">{stats?.verified || 0}</p>
                                <p className="text-xs text-green-600 mt-1">{t('active_accounts') || 'Active accounts'}</p>
                            </div>
                    ...
                    <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-xl shadow-sm border border-yellow-200 p-6 hover:shadow-md transition-all group">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-yellow-700">{t('unverified_customers') || 'Unverified'}</p>
                    ...
                    <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl shadow-sm border border-purple-200 p-6 hover:shadow-md transition-all group">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-purple-700">{t('customers_with_orders') || 'With Orders'}</p>
                                <p className="text-3xl font-bold text-purple-900 mt-2">{stats?.with_orders || 0}</p>
                                <p className="text-xs text-purple-600 mt-1">{t('active_shoppers') || 'Active shoppers'}</p>
                            </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1 relative">
                            ...
                            <input
                                ...
                                placeholder={t('search_customers') || 'Search customers by name, phone, or email...'}
                            ...
                            <tbody className="bg-white divide-y divide-slate-100">
                                {filteredCustomers.length > 0 ? (
                                    filteredCustomers.map((customer) => (
                                        <tr key={customer.id} className="hover:bg-slate-50/50 transition-all duration-150 group">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="relative">
                                                        <div className="w-12 h-12 bg-gradient-to-br from-purple-500 via-purple-600 to-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-lg shadow-purple-500/30 group-hover:scale-105 transition-transform">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    {customer.email && (
                                                        <div className="flex items-center gap-2 text-sm">
                                                            ...
                                                            <span className="text-slate-600 truncate max-w-xs">{customer.email}</span>
                                                        </div>
                                                    )}
                                                    {customer.address && (
                                                        <div className="flex items-center gap-2 text-sm">
                                                            ...
                                                            <span className="text-slate-600 truncate max-w-xs">{customer.address}</span>
                                                        </div>
                                                    )}
                                    ...
                                    <tr>
                                        <td colSpan="6" className="px-6 py-16 text-center">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <p className="text-slate-600 font-medium mb-1">{t('no_customers_found') || 'No customers found'}</p>
                                                <p className="text-sm text-slate-400">{t('try_different_search') || 'Try adjusting your search or filter'}</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                ...
                {filteredCustomers.length > 0 && (
                    <div className="text-center text-sm text-slate-600">
                        {t('showing_results') || 'Showing'} {filteredCustomers.length} {t('of') || 'of'} {customers?.length || 0} {t('customers') || 'customers'}
                    </div>
                )}
            </div>
        </AdminLayout>
