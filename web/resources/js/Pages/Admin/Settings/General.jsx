...
export default function GeneralSettings({ settings }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        default_estimated_delivery_time: settings?.default_estimated_delivery_time || 15,
    });
    ...
    return (
        <AdminLayout title={t('general_settings') || 'General Settings'}>
            ...
            <div className="space-y-6">
                {/* Header */}
                <div className="flex flex-col sm:flex-wrap items-start sm:items-center justify-between gap-4">
                    <div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="px-6 py-4 bg-gradient-to-r from-emerald-50 to-teal-50 border-b border-slate-200">
                            <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                                ...
                                {t('delivery_defaults_section_title') || 'Delivery Defaults'}
                            </h3>
                            <p className="text-sm text-slate-600 mt-1">
                                {t('delivery_defaults_section_hint') || 'Configure the estimated delivery time used across all orders (in minutes).'}
                            </p>
                        </div>
                        <div className="p-6 space-y-6">
                            <div>
                                <label className="block text-sm font-semibold text-slate-900 mb-2">
                                    {t('default_estimated_delivery_time_label') || 'Default estimated delivery time (minutes)'}
                                </label>
                                <div className="relative">
                                    <input
                                        type="number"
                                        min={1}
                                        max={240}
                                        value={data.default_estimated_delivery_time}
                                        onChange={(e) => setData('default_estimated_delivery_time', e.target.value)}
                                        className="w-full pl-4 pr-4 py-2.5 rounded-xl border border-slate-300 bg-white text-slate-700 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all"
                                    />
                                    <span className="absolute inset-y-0 right-4 flex items-center text-xs font-semibold text-slate-400">min</span>
                                </div>
                                <p className="text-xs text-slate-500 mt-2">
                                    {t('default_estimated_delivery_time_help') || 'This value is used when a store does not have its own delivery time configured.'}
                                </p>
                                {errors.default_estimated_delivery_time && (
                                    <p className="text-sm text-red-600 mt-1 flex items-center gap-1">
                                        <AlertCircle className="w-4 h-4" />
                                        {errors.default_estimated_delivery_time}
                                    </p>
                    ...
                    {/* Platform revenue */}
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="px-6 py-4 bg-gradient-to-r from-emerald-50 to-green-50 border-b border-slate-200">
                            <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                                <DollarSign className="w-5 h-5 text-emerald-600" />
                                {t('platform_revenue_settings') || 'Platform revenue'}
                            </h3>
                            <p className="text-sm text-slate-600 mt-1">
                                {t('platform_revenue_settings_hint') || 'Set how much the platform earns from each delivered order.'}
                            </p>
                        </div>
                        <div className="p-6 space-y-6">
                            <div>
                                <label className="block text-sm font-semibold text-slate-900 mb-2">
                                    {t('platform_commission_label') || 'Commission percentage'}
                                </label>
                                <div className="flex items-center gap-3">
                                    <input
                                        ...
                                        max="100"
                                        step="0.1"
                                        value={data.platform_commission_percentage}
                                        onChange={(e) => setData('platform_commission_percentage', e.target.value)}
                                        className="w-full px-4 py-2.5 rounded-xl border border-slate-300 bg-white text-slate-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all"
                                    />
                                    <span className="text-sm font-semibold text-slate-500">%</span>
                                </div>
                                <p className="text-xs text-slate-500 mt-2">
                                    {t('platform_commission_hint') || 'Applied to delivered orders when calculating store payouts.'}
                                </p>
